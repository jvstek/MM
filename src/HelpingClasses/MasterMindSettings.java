...
public class MasterMindSettings {
	...
	private float greyScoreValue; // give each there own value? or a standard value then increase it with multiplier? 
	...
	public MasterMindSettings(int PlayColors, int RowLenght, int MaxAttempt, int SameColorAllowed, float  GreyScoreValue, float WhiteScoreValue,float BlackScoreValue)throws SettingValidationError{
		if(!settingCheck(PlayColors, RowLenght, MaxAttempt, SameColorAllowed)){throw new SettingValidationError("SomeSettings aint good enough");}
		if (!checkValidation(PlayColors, RowLenght,SameColorAllowed)){throw new SettingValidationError("More Places in a row then Colors Availeble");}
		if(!checkScoreValue(GreyScoreValue, WhiteScoreValue,BlackScoreValue)){throw new SettingValidationError("valeu of grey needs to be smaller then white and white smaller then black");}
	...
	private boolean checkValidation(int Playcolors, int RowLenght,int SameColorAllowed){
		// if rowlength is longer then the playcolors * sameColorAllower game may not be made
	...
	private boolean checkScoreValue(float grey,float white,float black){
		...
		if(grey<white){return white<black;}
		return false;
	}
	private boolean settingCheck(int PlayColors, int RowLength,int MaxAttempt, int SameColorAllowed){
		if(PlayColors<3 || PlayColors>7){return false;}
		if(RowLength<3){return false;}
		if(MaxAttempt<4){return false;}
		if(SameColorAllowed<1){return false;}
		return true;
	}
