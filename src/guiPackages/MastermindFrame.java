...
public class MastermindFrame extends JFrame {
	...
	public void MenuItemClicked(ActionEvent e) {
		if (e.getSource() instanceof JMenuItem) {
			...
			case "Easy":
				...
				break;
			case "Normal":
				System.out.println("normal: todo");
				break;
			default:
				System.out.println("Action from unknown source");
				break;
			}
		}else{System.out.println("some other action  ");} 
	}
	...
	public void MenuGameClicked(){showPanelNumber(0,panels);}
	private boolean containsScoreBoard(JPanel[] panels){
		for (JPanel jPanel : panels) {
			if(jPanel instanceof ScoreBoard){return true;}
		}
		return false;
	}
	private void showPanelNumber(int panelNumber,JPanel[] panels){
		for (int i = 0; i < panels.length; i++) {
			if(i ==panelNumber){panels[i].setVisible(true);}else{panels[i].setVisible(false);}
		}
